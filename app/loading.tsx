  ...
  return (
    <div className="relative flex min-h-[80vh] w-full flex-col items-center justify-center gap-10 overflow-hidden px-6">
      ...
      <div className="relative flex h-44 w-44 items-center justify-center">
        ...
        <div className="absolute inset-0 rounded-full border border-customOrange/30 fitrock-orbit">
        ...
        <div
          className="absolute inset-3 rounded-full border border-customOrange/20 fitrock-orbit-reverse"
        >
        ...
        <svg
          viewBox="0 0 120 60"
          className="relative z-10 h-20 w-20 text-customOrange fitrock-lift drop-shadow-[0_4px_18px_rgba(228,124,72,0.45)]"
          fill="currentColor"
          aria-hidden="true"
        >
      ...
      <div className="flex flex-col items-center gap-4">
        <div className="flex gap-1 text-2xl font-bold tracking-[0.35em] text-neutral-900 md:text-3xl">
          {letters.map((letter, i) => (
            <span
              key={i}
              className="inline-block fitrock-wave"
        ...
        <div className="relative mt-2 h-1 w-56 overflow-hidden rounded-full bg-neutral-200">
          <span className="absolute inset-y-0 left-0 w-1/3 rounded-full bg-gradient-to-r from-transparent via-customOrange to-transparent fitrock-progress" />
        </div>
      </div>

      <style>{`
        @keyframes fitrock-orbit {
          from { transform: rotate(0deg); }
          to { transform: rotate(360deg); }
        }
        @keyframes fitrock-orbit-reverse {
          from { transform: rotate(360deg); }
          to { transform: rotate(0deg); }
        }
        @keyframes fitrock-lift {
          0%, 100% { transform: translateY(0) rotate(-6deg); }
          50% { transform: translateY(-6px) rotate(6deg); }
        }
        @keyframes fitrock-wave {
          0%, 60%, 100% { transform: translateY(0); opacity: 0.55; }
          30% { transform: translateY(-8px); opacity: 1; color: #e47c48; }
        }
        @keyframes fitrock-progress {
          0% { transform: translateX(-100%); }
          100% { transform: translateX(400%); }
        }
        .fitrock-orbit { animation: fitrock-orbit 3.2s linear infinite; }
        .fitrock-orbit-reverse { animation: fitrock-orbit-reverse 4.4s linear infinite; }
        .fitrock-lift { animation: fitrock-lift 1.4s ease-in-out infinite; transform-origin: center; }
        .fitrock-wave { animation: fitrock-wave 1.6s ease-in-out infinite; }
        .fitrock-progress { animation: fitrock-progress 1.6s ease-in-out infinite; }
      `}</style>
    </div>
  );
}
